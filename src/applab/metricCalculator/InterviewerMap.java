...
public class InterviewerMap {
        ...
        if (result.getSize() > 0) {
            while (moreRows) {
                ...
                for (int i = 0; i < people.length; i++) {
                    ...
                    if (!singletonValue.districtMap.containsKey(person.getDistrict__r().getName().toUpperCase())) {
        ...
        if (result.getSize() > 0) {
            while (moreRows) {
                ...
                for (int i = 0; i < ckws.length; i++) {
                    ...
                    if (!singletonValue.districtMap.containsKey(ckw.getPerson__r().getDistrict__r().getName())) {
    ...
    public static Double getTotalSubmissions(Integer surveyId, String key) throws SQLException {
        ...
        HashMap<String, Double> map = singletonValue.submissionCount.get(surveyId);
        if (map == null) {
    ...
    private static String getSurveyIdsQuery(Integer dashboardId) {
        ...
        commandText.append("FROM ");
        commandText.append(DatabaseHelpers.METRIC_PARAMETER_TABLE);
        commandText.append(" WHERE ");
