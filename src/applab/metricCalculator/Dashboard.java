...
public class Dashboard {
     ...
     *
     * @return - List o metrics to upload
     */
    public ArrayList<M_E_Metric_Data__c> getDatas() throws SQLException, RemoteException, ServiceException {
        ...
        HashMap<String, M_E_Metric_Data__c> totalDatas = getMetricDatas();
        ...
        for (Entry<String, MetricCalculation> entry : this.calcualtionMap.entrySet()) {
            ...
            if (parameter.getOnlyAnsweredSurveys().equals("I")) {
                total = this.totalGeneralDetailMap.get(subDivider).getTotalInterviewer();
            }
            else if (parameter.getOnlyAnsweredSurveys().equals("Y")) {
                total = parameter.getTotalSubmissions(subDivider);
            }
            else {
                total = InterviewerMap.getTotalSubmissions(parameter.getSurveyId(), subDivider);
            }
            ...
            if (data == null) {
                ...
                entry.getValue().updateMetricDataValue(parameter, total);
                data = entry.getValue().getData();
            }
            datas.add(data);

            // Add to the total metric calculation
            MetricCalculation totalCalculation = this.totalCalcualtionMap.get(entry.getValue().getMetricName());
            if (totalCalculation == null) {
                totalCalculation = new MetricCalculation(
                        null,
                        entry.getValue().getParameterName(),
                        entry.getValue().getMetricName()
                );
                totalCalculation.addMetricData(totalDatas.get(entry.getValue().getMetricName()));
            }
            totalCalculation.addToValue1(data.getActual_Value__c());
            if (parameter.getCalculationType().toString().equals("percentage")) {
                totalCalculation.addToValue2(1.0);
            }
            else {
                totalCalculation.addToValue2(total);
            }
            this.totalCalcualtionMap.put(entry.getValue().getMetricName(), totalCalculation);
        }
    ...
    private ArrayList<M_E_Metric_Data__c> calculateTotals() throws RemoteException, ServiceException, SQLException {
        ...
        for (Entry<String, MetricCalculation> entry : this.totalCalcualtionMap.entrySet()) {
            ...
            if (data == null) {
                ...
                entry.getValue().updateMetricDataValue(this.parameterMap.get(entry.getValue().getParameterName()), -1.0);
                data = entry.getValue().getData();
            }
            datas.add(data);
        }
     ...
     *
     * @return - HashMap of the metric datas that are for the totals. This is done as the totals are yet to be calculated
     */
    private HashMap<String, M_E_Metric_Data__c> getMetricDatas() throws RemoteException, ServiceException, SQLException {

        if (this.partnerName == null) {
            this.partnerName = DatabaseHelpers.getPartnerName(this.dashboardId);
        }
        HashMap<String, M_E_Metric_Data__c> totalDatas = new HashMap<String, M_E_Metric_Data__c>();
        ...
        if (result.getSize() > 0) {
            while (moreRows) {
                ...
                for (int i = 0; i < datas.length; i++) {
                    ...
                    String key = Utils.createMetricLabel(data.getM_E_Metric__r().getName(), districtString);
                    if (this.calcualtionMap.containsKey(key)) {
                        this.calcualtionMap.get(key).addMetricData(data);
                    }
                    else if (districtString == null) {
                        totalDatas.put(data.getM_E_Metric__r().getName(), data);
                    }
                }
                if (result.isDone()) {
                    moreRows = false;
